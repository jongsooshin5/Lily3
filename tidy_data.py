...
def wind_pre_proc(dataset):
    """
    Preprocess wind data to be consistent with other datastreams: reformat lat/lon values, remove data over land
    Inputs:
    - dataset: xarray dataset, read in from read_file with coordinates (time, lat, lon)
    Outputs:
    - dataset: xarray dataset, processed w above changes
    """
    ds = dataset
...
def get_contours(file):
    """
    Gets dynamic topography contours from input file
    Inputs:
    - file: Path to netCDF file containing data, generated on vortex
    Outputs:
    - adt_mean: xarray dataset, formated in (time,lat,lon) dimensions with climatology, seasonal trend, and linear trend removed
    """
    ds_contours = xr.open_dataset(file)  # Read netcdf data into xarray
