...
adt_mean = get_contours(f)
gsi_lon, gsi_lat, sla_ts, sla_ts_std = gs_index(ds,adt_mean*100)
...
num_modes = 4
bbox      = [280, 308, 33, 46]
adt_slice = adt_mean[np.nanargmin(abs(ds.latitude-bbox[2])):np.nanargmin(abs(ds.latitude-bbox[3])),
...
spatial_plot(ds.longitude, ds.latitude, np.nanstd(ds.sla,axis=0)*100,bthy_data= abs(adt_mean)*100,
             levels = [get_max_contour(ds,adt_mean*100)], x_gsi = gsi_lon, y_gsi = gsi_lat,
